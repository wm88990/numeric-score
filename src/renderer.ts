...
function renderMelodyToken(token: Token, x: number, y: number): RenderResult {
  ...
  for (const c of chars) {
    ...
    if (c.type === 'note') {
      ...
      svg += `<circle cx="${cx + w / 2}" cy="${y - 5}" r="2.5" fill="${C.melodyColor}"/>`;
    } else if (c.type === 'dash') {
  ...
  if (beam > 0 && firstNoteX >= 0) {
    const x1 = firstNoteX - 7;
    const x2 = lastNoteX + 7;
    for (let i = 0; i < beam; i++) {
      const by = C.underlineY + i * C.underlineGap;
      svg += `<line x1="${x1}" y1="${by}" x2="${x2}" y2="${by}" stroke="${C.melodyColor}" stroke-width="1.5"/>`;
    }
  }

  return { width: cx - x + C.tokenGap, svg };
}
...
function renderBarline(type: string, x: number, y: number): string {
  ...
  if (type === 'end') {
    ...
    return `<line x1="${x - C.barlineGap}" y1="${top}" x2="${x - C.barlineGap}" y2="${bot}" stroke="${C.barlineColor}" stroke-width="${C.endBarlineW}"/>` +
           ...
           `<circle cx="${x - 10}" cy="${y + 35}" r="2.5" fill="${C.barlineColor}"/>` +
           `<circle cx="${x - 10}" cy="${y + 48}" r="2.5" fill="${C.barlineColor}"/>`;
  } else if (type === 'repeat-end') {
    return `<circle cx="${x - 12}" cy="${y + 35}" r="2.5" fill="${C.barlineColor}"/>` +
           `<circle cx="${x - 12}" cy="${y + 48}" r="2.5" fill="${C.barlineColor}"/>` +
...
function renderMeasure(measure: Measure, x: number, y: number): RenderResult {
...
export function renderScore(score: Score): string {
  ...
  for (const layout of layouts) {
    ...
    if (layout.lyric) {
      svg += `<text x="${x + 50}" y="${layout.y + C.lyricY}" font-size="${C.lyricFS}" fill="${C.lyricColor}" font-family="KaiTi, serif">${esc(layout.lyric)}</text>`;
    }

    for (const measure of layout.measures) {
      const r = renderMeasure(measure, x, layout.y);
      svg += r.svg;
      x += r.width;
    }
  }
